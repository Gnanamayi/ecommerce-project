...
// Include database connection file (replace with actual path)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $roles = $_POST['roles'];  // This is the role input (e.g., buyer, seller, admin)
    ...
    if (empty($name) || empty($email) || empty($password) || empty($roles)) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (user_name, user_email, user_password, roles) VALUES (?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['roles'] = $roles;  // Store the user's role in session
            $_SESSION['logged_in'] = true;

            // Redirect based on role
            if ($roles == 'seller') {
                header('location: seller.php');  // Redirect to seller.php if the role is seller
                exit();
            } else if ($roles == 'admin') {
                header('location: admin.php');  // Redirect to admin.php if the role is admin
                exit();
            } else {
                header('location: account.php?register_success=Registration successful');  // Redirect to account page for buyer
                exit();
            }
        } else {
            // Handle error in insertion
            $error = "There was an error in registering your account.";
        }
    }
}
?>
